...
Формат — Conventional Commits (`type(scope): описание`), `type`/`scope` на английском, описание и тело — на русском. По умолчанию один коммит на логическую задачу и без лишних слов: не растягивать тело построчным пересказом диффа и не дробить на несколько коммитов, если явно не попросили.

### Pull Request

Шаблон — `.github/pull_request_template.md`. Пишется для ревьюера: «что и зачем», «что сделано», «как проверить», без пересказа диффа. Архитектурное решение внутри PR — не пересказывать, ссылка на ADR (`docs/adr/`) или дневник задачи (`docs/features/`). Заголовок PR — в формате Conventional Commits, как коммиты (при squash-merge он станет сообщением коммита).

## Документация решений и изменений

Этот файл хранит только текущее состояние проекта — не историю. История и рабочий процесс фиксируются отдельно, в двух местах с разной зоной ответственности:

- **`docs/adr/`** — архитектурные решения: границы модулей, выбор технологии, сознательно отклонённые варианты. Перед пересмотром архитектурного вопроса — сначала проверить `docs/adr/README.md` (правила, индекс), не решался ли он уже. См. [ADR-0001](docs/adr/0001-claude-md-and-adr-process.md).
- **`docs/features/`** — рабочий дневник конкретной задачи/фичи, любого размера. Статус, цель, план, что сделали, что не стали. Если по ходу фичи принято архитектурное решение — оно **не пересказывается** в feature-файле, а оформляется отдельным ADR со ссылкой. Правила и шаблон — [`docs/features/README.md`](docs/features/README.md).

Коротко: ADR отвечает на «почему устроено именно так», feature-лог — на «что и зачем сделали в рамках этой задачи», `CLAUDE.md` — на «как это устроено сейчас».

**Обязательный рабочий процесс для Claude Code:**
1. Когда пользователь просит спланировать изменения — прежде чем предлагать план, проверить: есть ли уже открытый (🟡) файл в `docs/features/` под эту задачу. Если нет — напомнить об этом и создать файл сразу, не откладывая на конец работы.
2. План фиксируется в файле сразу, не только в чате. По ходу работы разделы «Сделано»/«Не стали делать» дополняются вживую — черновые формулировки на этом этапе нормальны. Перед сменой статуса на ✅/⏸️/❌ — обязательный проход почистить лишнее.
3. Файл в `docs/features/`/`docs/adr/` всегда перечитывать заново перед правкой, не полагаться на то, что помню его содержимое с начала сессии — пользователь может редактировать эти файлы напрямую.
4. Если по ходу работы напрашивается архитектурное решение (см. критерии в `docs/adr/README.md`) — предложить оформить его отдельным ADR, не пересказывать в feature-файле.
5. После добавления нового ADR или feature-файла — сразу актуализировать соответствующий индекс (`docs/adr/README.md` / `docs/features/README.md`), это часть создания записи.
